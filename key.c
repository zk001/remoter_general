...
static key_state_t   key_status[MAX_KEY];
...
void cal_key_press_time(key_state_t * key_result, key_status_t key_s, key_index_t key)
{
...
void key_read(key_state_t * key_result, const key_type_t* key)
{
...
bool is_key_pressing(u8 key, u32 *time)
{
  key_state_t key_f;
...
bool is_key_released(u8 key, u32 *time)
{
  key_state_t key_f;
...
bool is_key_on(u8 key, u32 time)
{
  key_state_t key_f;
...
void get_key_combin_setup_time_and_combin_time(u8 key, u32 *st_time, u32 *cmb_time)
{
...
bool is_key_valid(u8 key, u32 time)
{
  key_state_t key_f;
...
bool is_leader_key_event_finish()
{
  u32 second_key_time;
...
void set_key_event(u8 key, key_action_t key_ac)
{
  key_event[key].key_current_action  |= key_ac;
}

bool is_short_immediately_key(u8 i, key_state_t* key_s)
{
	if((key_s->cur_status == PRESSING) && (key_s->pressing_time))
		reload_sys_time();

  if((key_s->cur_status == PRESSING) && (key_s->pressing_time) && !(key_s->key & SHORT_KEY_IMMEDIATELY_FLAG)){///bit29 is the flag that indication the short key only once,bit30 indication not stuck key
...
//    reload_sys_time();
//    aw9523_init();
//    HalLedSet (T_KEY_ERTONGQINGXI_LED, HAL_LED_MODE_ON);
...
bool is_short_released_key(u8 i, const key_state_t* key_s)
{
  event_handler_t *pos_ptr = NULL;
...
bool is_long_key(u8 i, key_state_t* key_s)
{
  event_handler_t *pos_ptr = NULL;
  u32  key_handler_time1 = 0;
  u32 default_time;
...
bool is_stuck_key(key_state_t* key_s)
{
  return ((key_s->cur_status == PRESSING) && \
      (key_s->pressing_time > STUCK_TIME) && \
...
bool is_combin_key(u8 i, key_state_t* key_s)
{
  event_handler_t *pos_ptr = NULL;
  u32  key_handler_time1 = 0;
...
int key_process(void *data)
{
  ...
  static u8 is_two_key_pressed = 0;
  static u8 setup_time_valid= 0;
  static u8 invalid_key  = 0;
  ...
  for(u8 i = 0; i < MAX_KEY; i++){
    ...
    is_short_immediately_key(i, key_s);
...
key_action_t get_key_status(u8 key)
{
  return key_event[key].key_current_action;
}

void clr_key_status(u8 key, u32 status)
{
  key_event[key].key_current_action &= ~status;
}

handler get_key_handler(u8 key, key_action_t key_ac, u8 second_key)
{
  event_handler_t *pos_ptr = NULL;
